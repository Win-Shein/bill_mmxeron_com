...
const SESSION_SECRET = process.env.SESSION_SECRET || (isProd ? null : 'billing-dev-secret-change-me');
if (!SESSION_SECRET) {
...
app.get('/api/branding', (req, res) => {
  const s = db.prepare('SELECT company_name, logo_url FROM settings LIMIT 1').get();
  res.json({ company_name: (s && s.company_name) || '', logo_url: (s && s.logo_url) || '' });
});
