...
function buildInvoicePdf(inv, settings, out) {
  ...
  // A draft only has a temporary placeholder number (DRAFT-<uuid>) until it is
  // issued, so never print that internal reference on the client-facing PDF.
  const isDraftNo = /^DRAFT-/i.test(String(inv.invoice_no || ''));
  if (!isDraftNo) {
    doc.fillColor(dark).font('Helvetica').fontSize(10)
      .text(`Invoice No: ${inv.invoice_no}`, 320, isStorno ? 76 : 82, { width: 225, align: 'right' });
  }

  let metaY = isDraftNo ? (isStorno ? 76 : 82) : (isStorno ? 92 : 100);
  ...
  if (settings.bank_iban || settings.bank_name) {
    ...
    y += 12;
    const bankLines = [
      settings.bank_name ? `Pay Name: ${settings.bank_name}` : null,
      settings.bank_account_holder ? `Account Name: ${settings.bank_account_holder}` : null,
      settings.bank_iban ? `Account No: ${settings.bank_iban}` : null,
      isDraftNo ? null : `Reference: ${inv.invoice_no}`,
    ].filter(Boolean);
