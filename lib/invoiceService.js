...
const replaceLineItems = db.transaction((invoiceId, lines) => {
  ...
  const insert = db.prepare(
    `INSERT INTO invoice_items
       (invoice_id, item_id, description, category, quantity, unit_price, tax_rate,
        line_total, service_start, service_end, sort_order)
     VALUES (@invoice_id, @item_id, @description, @category, @quantity, @unit_price,
             @tax_rate, @line_total, @service_start, @service_end, @sort_order)`
  );
  lines.forEach((l, idx) => {
    ...
    insert.run({
      ...
      service_start: l.service_start || null,
      service_end: l.service_end || null,
      sort_order: idx,
    });
  });
});
...
/**
 * Legacy temporary placeholder for an invoice that has no real number yet.
 * New invoices reserve their sequential number at creation (see routes/invoices.js);
 * this remains for older drafts and is replaced by a real number when issued.
 */
 ...
 *  - keeps its sequential invoice number (assigning one only if it still has
 *    a temporary DRAFT- placeholder)
 ...
 *
 * Requires the invoice to already have a customer and at least one line item.
 */
const issueInvoice = db.transaction((id, orgId) => {
  ...
  // The per-invoice service period (Leistungszeitraum) is derived from the
  // line items when it wasn't set explicitly: earliest start, latest expiry.
  const starts = items.map((it) => it.service_start).filter(Boolean).sort();
  const ends = items.map((it) => it.service_end).filter(Boolean).sort();
  const servicePeriodStart = inv.service_period_start || (starts.length ? starts[0] : null);
  const servicePeriodEnd = inv.service_period_end || (ends.length ? ends[ends.length - 1] : null);
  ...
  // Keep the number reserved at creation time; only assign a fresh one for
  // legacy drafts that still carry a temporary DRAFT- placeholder.
  const hasRealNo = !/^DRAFT-/i.test(String(inv.invoice_no || ''));
  const year = (inv.issue_date || new Date().toISOString().slice(0, 10)).slice(0, 4);
  const invoiceNo = hasRealNo ? inv.invoice_no : nextInvoiceNo(orgId, year);
  db.prepare(
    `UPDATE invoices
        SET invoice_no = @invoice_no, client_country = @client_country,
            ...
            issuer_tax_number = @issuer_tax_number, vat_rate = @vat_rate,
    ...
    issuer_tax_number: settings.tax_number || null,
...
const cancelInvoice = db.transaction((id, orgId) => {
  ...
  const info = db.prepare(
    `INSERT INTO invoices
       (org_id, invoice_no, customer_id, issue_date, due_date, status, currency, discount,
        notes, terms, issuer_tax_number, client_country, service_period_start,
  ...
  items.forEach((it, idx) => {
    ...
    insertLine.run({
      ...
      service_start: it.service_start,
      service_end: it.service_end,
      sort_order: idx,
    });
  });
